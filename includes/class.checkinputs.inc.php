...
include_once("class.fechas.inc.php");
...
class cCheckInput {
	...
	private static $rxemail = "/^(?:[a-z0-9!#$%&'*+\/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+\/=?^_`{|}~-]+)*|\"(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21\x23-\x5b\x5d-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])*\")@(?:(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?|\[(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?|[a-z0-9-]*[a-z0-9]:(?:[\x01-\x08\x0b\x0c\x0e-\x1f\x21-\x5a\x53-\x7f]|\\[\x01-\x09\x0b\x0c\x0e-\x7f])+)\])$/iD"; // As per RFC 5322
...
/* Esto tanto sirve para validar nombres de usuarios como contraseñas */
	...
	static function StrongPassword($string, $index, $label) {
		$result = false; // Soy pesimista
		if ((strlen($string) == 0) and (self::$checkempty)) {
			self::putMsgErr('pass',0,($label!=null)?$label:$index, $label);
		} else {
			if (mb_strlen($string) > 0) {
				if (mb_strlen($string) < 2) {
					self::putMsgErr('pass',1,($label!=null)?$label:$index, $label);
				} else {
					if (mb_strlen($string) > 256) {
						self::putMsgErr('pass',2,($label!=null)?$label:$index, $label);
					} else {
	...
	static function CPA($string,$index,$label = null) {
		$result = false;
		self::$currentString = $string;
		if ((strlen($string) == 0) and (self::$checkempty)) {
