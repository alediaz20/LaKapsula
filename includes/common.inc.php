...
function EnsureTailingSlash($str)
{ // Alias de la anterior.
...
function EnsureTailingURISlash($str)
{ // Alias de la anterior.
...
function GetIP()
{
	...
	if (!ValidIP($result)) {
		if (!empty($_SERVER["HTTP_X_FORWARDED"]) and ValidIP($_SERVER["HTTP_X_FORWARDED"])) {
			$result =  $_SERVER["HTTP_X_FORWARDED"];
		} else {
			if (!empty($_SERVER["HTTP_FORWARDED_FOR"]) and ValidIP($_SERVER["HTTP_FORWARDED_FOR"])) {
				$result = $_SERVER["HTTP_FORWARDED_FOR"];
			} else {
				if (!empty($_SERVER["HTTP_FORWARDED"]) and ValidIP($_SERVER["HTTP_FORWARDED"])) {
...
function ParseMetadata($metadata, $alias = null)
{
	...
	if (is_object($metadata)) {
		...
		if (!isset($metadata->hasmainmenu)) {
			$metadata->hasmainmenu = true;
		} // Por omisión Sí
		...
		if (!isset($metadata->hassubmenu)) {
			$metadata->hassubmenu = false;
		} // Por omisión No
...
*/
/**
 * Summary. Genera una codigo aletorio
 * @return string $result
 */
function genToken()
{
	$token =	substr(str_shuffle('0123456789abcdfghijklmnopqrstuvwxyz'), 4, 10);
	$result =	substr(md5($token), 0, rand(2, 3)) . substr(str_shuffle('abcdfghijklmnopqrstuvwxyz'), 3, 8);
	return $result;
}
/**
...
function IsValidState($estado)
{
	if (!defined("ESTADOS_VALIDOS")) {
		define(
			"ESTADOS_VALIDOS",
			['HAB' => 'Habilitado', 'DES' => 'Deshabilitado', 'ELI' => 'Eliminado']
		);
	}
	return array_key_exists($estado, ESTADOS_VALIDOS);
}
