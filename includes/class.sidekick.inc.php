...
include_once("class.checkinputs.inc.php");
...
*/
	 ...
	 * @param str $type - Tipo de mensaje a mostrar: Warning, info o danger.
	 * Esto modifica el ícono a mostrar en el mensaje.
	 * @param str $msg El mensaje a mostrar
	 */
	
	static function ShowWarning($msg, $type="warning")
	{
		$types = [
		...
		$typeMessage = $types['warning'];
		if(isset($types[$type])){ $typeMessage = $types[$type]; }
...
*/
	...
	/**
	 * Muestra un mensaje tipo card para mostrar al usuario
	 * @param string $msg : (opc) Mensaje para mostrar.
	 */
	
	static function ShowMessage($msg = "No hay resultados para mostrar"){
		$nombre_plantilla = "showMessage.htm";
		if (ExisteArchivo(DIR_plantillas . $nombre_plantilla)) {
			include(DIR_plantillas . $nombre_plantilla);
		} else {
			echo '<p>' . $msg . '</p>';
